...
class TreeModel {
	...
	private function _toFormatTree(&$list,$level=0,$title = 'title') {
		foreach($list as $key=>$val){
			...
			//$val['title_show'] =$level==0?"&nbsp;":$tmp_str."&nbsp;";
		...
		private function _toTree(&$list,$level=0,$title = 'title') {
		foreach($list as $key=>$val){
			$tmp_str=str_repeat("&nbsp;",$level*2);
			$tmp_str.="└";
			...
			if(!array_key_exists('_child',$val)){
				...
				unset($val['_child']);
				array_push($this->formatTree,$val);
				   $this->_toTree($tmp_ary,$level+1,$title); //进行下一层递归
				}
			}
			return;
		}
		...
		public function formatTree(&$list,$title = 'title',$pk='id',$pid = 'pid',$root = 0){
			$list = list_to_tree($list,$pk,$pid,'_child',$root);
			$this->formatTree = array();
			$this->_toTree($list,0,$title);
			return $this->formatTree;
		}
	}
