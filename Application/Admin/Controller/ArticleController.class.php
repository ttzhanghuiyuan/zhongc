...
class ArticleController extends AdminController {
    ...
    protected function getMenu(){
        ...
        if(empty($this->cate_id))
        {
            foreach ($cate as $key => $value) {
                ...
                foreach ($value['_child'] as $key => $value) {
    ...
    {
        $cate =M('Category')->where(array('status'=>1))->field('id,title,pid,allow_publish')->order('pid,sort')->find();
        $cateid=$cate['id'];
        $this->redirect('Article/index',array('cate_id'=>$cateid));
    }*/
