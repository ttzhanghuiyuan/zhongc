 ...
 */
class GoodcatesController extends AdminController {
    ...
    public function index(){
        $tree = D('Goodcates')->getTree(0,'hg_goodcates.application,hg_goodcates.id,hg_goodcates.name,hg_goodcates.sort,hg_goodcates.pid,hg_goodcates.show,picture.path');
    ...
    public function edit($id = null, $pid = 0){
        ...
        if(IS_POST){ //提交表单
            ...
            if($pid){
                /* 获取上级分类信息 */
                $cate = $Category->info($pid, 'id,name,sort');/*,title,status*/
    ...
    public function add($pid = 0){
        ...
        if(IS_POST){ //提交表单
            ...
            if($pid){
                ...
                $cate = $Category->info($pid, 'id,name,sort');/*,title,status*/
                if(!($cate )){
                    $this->error('指定的上级分类不存在或被禁用！');/*&& 1 == $cate['status']*/
                }
            }
    ...
    public function remove(){
        ...
        $child = M('Goodcates')->where(array('pid'=>$cate_id))->field('id')->select();
        if(!empty($child)){
        ...
        //删除该分类信息
        $res = M('Goodcates')->delete($cate_id);
        if($res !== false){

            $arr=M('Good')->where('goodcates_id='.$cate_id)->getField('id',true);

            $map['id']=array('in' , arr2str($arr));

            if(!empty($arr)){
                M('Good')->where($map)->setField('goodcates_id',0);
            }

            S('sys_goodcates_list', null);

            S('sys_goodcates_list',D('Goodcates')->getTree(0,'hg_goodcates.application,hg_goodcates.id,hg_goodcates.name,hg_goodcates.sort,hg_goodcates.pid,hg_goodcates.show,picture.path'));
            //记录行为
            action_log('update_goodcates', 'goodcates', $cate_id, UID);
            $this->success('删除分类成功！');
        }else{
            $this->error('删除分类失败！');
        }
