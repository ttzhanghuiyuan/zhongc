...
class AdminController extends CommonController {
    ...
    final protected function edittable($table,$map,$data,$jumpurl)
    {

        if( M($table)->where($map)->save($data)!==false ) {
            if($jumpurl==null){
                $this->success('操作成功!');
            }
            else
    ...
    final public function getMenus($controller=CONTROLLER_NAME){
         $menus  =   S('ADMIN_MENU_LIST'.$controller);
        if(empty($menus)){
            // 获取主菜单
            $where['pid']   =   0;
            $where['hide']  =   0;
            ...
            if($current){
             ...
             S('ADMIN_MENU_LIST'.$controller,$menus);
        }
        return $menus;
    }
    ...
    protected function lists ($model,$where=array(),$order='',$base = array('status'=>array('egt',0)),$field=true){
        ...
        if( isset($REQUEST['r']) ){
            ...
            $listRows = C('LIST_ROWS') > 0 ? C('LIST_ROWS') : 40;
        }
    ...
    public function childlist()
    {
        ...
        if($_GET['id']){
            ...
            $data=M($tb)->where('pid='.$_GET['id'])->order('sort desc')->field('id,name')->select();
