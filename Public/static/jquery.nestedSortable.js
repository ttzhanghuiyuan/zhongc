		...
		toHierarchy: function(options) {
			...
			function _recursiveItems(item) {
				...
				if (id) {
					...
					var nav_name = $(item).find('input[name=nav_name]').val();
					var nav_target = $(item).find('input[name=target]');
					nav_target = nav_target.is(":checked") ? 1 :0;

					var nav_type = $(item).find('input[name=nav_type]').val();
					var concern_id = $(item).find('input[name=concern_id]').val();
					var nav_remark = $(item).find('input[name=remark]').val();

					var currentItem ={ "id": id, "nav_url" : nav_url, "nav_name" : nav_name , "nav_target" : nav_target , "nav_type" : nav_type, 'concern_id': concern_id, 'nav_remark' : nav_remark }; //{"id" : id};
		...
		toForms: function(options) {
			...
			function _recursiveItems(item) {
				...
				if (id) {
					...
					var input_rule = $(item).find('select[name=input_rule] option:selected').val();
					...
					if(input_type=="2"){
						...
						$(item).find('.tab_panel .option input').each(function(_index,_item){

							option.push( { 'value':i , 'name':$(_item).val() });
							
							i++;

						});

						currentItem ={ "id": id, "input_name" : input_name ,  "input_type" : input_type,  'input_rule' : input_rule, 'option' : option };


					}else{
						currentItem ={ "id": id, "input_name" : input_name ,  "input_type" : input_type,  'input_rule' : input_rule };
					}

					//currentItem ={ "id": id, "input_name" : input_name ,  "input_type" : input_type,  'input_rule' : input_rule }; //{"id" : id};
		...
		toArray: function(options) {
			...
			function _recursiveArray(item, depth, left) {
				...
				if (id) {
					...
					ret.push({ "id": id, "pid": pid==null?0:pid, "nav_url" : nav_url, "nav_name" : nav_name , "nav_target" : nav_target , "nav_type" : nav_type, 'concern_id': concern_id, 'nav_remark' : nav_remark });//"depth": depth, "left": left, "right": right
				}
