...
"proportional" =>false,  //1 / 1, //比例截图，如果你是动态比例，请设为false，false时就不会验证JS端的比例了。如果为数字时，JS端也要相应设置哦，不然系统会给你抱出错误,不设比例填0，如填比例 ：3/4  代表宽和高的比例是3/4(3除以4的意思懂吗，菜菜，你可直接填0.75，没错)
...
array(-1,true,""),//此时的-1   代表以原图为基准，获得截图
...
), "water" => false, //"../images/waterimg2.png", //只接受PNG水印，当然你对PHP熟练，你可以对主程序进行修改支持其他类型水印,不设就"water"=>false
...
"saveURL" =>$_SERVER['DOCUMENT_ROOT'].'/Uploads/Download/', //ShearURL . "file" . DIRECTORY_SEPARATOR . "shearphoto_file" . DIRECTORY_SEPARATOR, //截好后的图片。储存的目录位置，后面不要加斜杠，系统会自动给补上！不要使用中文

"filename" => date("YmdHis",time()). mt_rand(100, 999)// uniqid("shearphoto_") . "_" . mt_rand(100, 999) //. "_"//截好后的图片的文件名字定义！要生成多个文件时 系统会自动在后面补上  "width"=>array()参数定义的名称,请查看上面的"width"=>array()
);
