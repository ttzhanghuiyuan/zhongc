...
window.ShearPhoto.MINGGE(function() {
	...
	Shear.config({
		...
		preview: false, //[150], // 开启动态预览图片 (数组成员整数型，禁止含小数点 可选false 和数组)     数组内是宽度设置，没有高度设！因为高度会按比例做事 ，此设置代表预览150 大小的预览图（你可以增加多个预览图,如[100,70,50]），设置越多预览图,shearphoto性能越差！官方不建意你开启这个功能，尽可能请设为preview:false
		...
		proportional: [
			0,//1 / 1, <!--截框的宽高比例（宽除以高的比例值，这个设置其实就是1，对！你可以直接写1  如填3/4 那么就是0.75的比例,不设比例请设为0，注意更改比例后，后端也要进行相应设置，否则系统会给你抱出错误-->
			/*
		...
		scope: document.getElementById("mainpic"), //main范围对象 
	...
	Shear.complete = function(serverdata) { //截图成功完成时，由shearphoto.php返回数据过来的成功包
			...
			// SendArry = {};
			// SendArry.imgurl = serverdata[0]['ImgUrl'];
			// SendArry.imgname = serverdata[0]['ImgName'];

			// Shear.MyAjax.carry({
			// 	url: document.getElementById('posturl').value,
			// 	data: "savedata =" + window.ShearPhoto.JsonString.JsonToString(SendArry),
			// 	type: "POST",
			// 	timeout: 9e4,
			// 	async: true,
			// 	lock: true,
			...
			// 		data = window.ShearPhoto.JsonString.StringToJson(data);
			// 		if (data.status === 2) {
			...
			// 		}
			// 		// if (data["erro"]) {
			// 		// 	this_.SendUserMsg("错误：" + data["erro"], 5e3, 0, "#f4102b", "#fff", false);
			// 		// 	return;
			// 		// }
			...
			for (var i = 0; i < length; i++) {
				...
				creatImg.src = serverdata[i]["ImgUrl"];// this.arg.relativeUrl + serverdata[i]["ImgUrl"];
			}
	...
	try {
		...
		for (var i = 0; i < photoalbumLi.length; i++) 
			photoalbumLi[i].onclick = photoalbumLifun; //为相册的每张照加入一个点击事件
